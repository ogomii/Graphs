...
class Converter:
    ...
    @staticmethod
    def sequence_to_adj_list(sequence: list) -> dict:
        adj_list = defaultdict(list)
        sequence = sequence.copy()
        sequence.sort(reverse=True)

        # 4 2 2 3 2 1 4 2 2 2 2
        
        ver_degree: list = []

        for i in range(0, len(sequence)):
            ver_degree.append([i, sequence[i]])

        for i in range(0, len(sequence)):
            adj_list[i + 1]
            
            first_ver = ver_degree[0][0]
            for j in range(0, ver_degree[0][1] + 1):
                second_ver = ver_degree[j][0]

                if not first_ver == second_ver:
                    adj_list[first_ver + 1].append(second_ver + 1)
                    adj_list[second_ver + 1].append(first_ver + 1)

                ver_degree[j][1] = ver_degree[j][1] - 1

            ver_degree[0][1] = 0
            ver_degree.sort(reverse=True, key=lambda degree: degree[1])
